...
func (_q *UserQuery) QueryMemberships() *CourseMembershipQuery {
...
// QueryOwnedCourses chains the current query on the "owned_courses" edge.
func (_q *UserQuery) QueryOwnedCourses() *CourseQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOwnedCourses:    _q.withOwnedCourses.Clone(),
...
func (_q *UserQuery) WithMemberships(opts ...func(*CourseMembershipQuery)) *UserQuery {
	query := (&CourseMembershipClient{config: _q.config}).Query()
...
// WithOwnedCourses tells the query-builder to eager-load the nodes that are connected to
// the "owned_courses" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithOwnedCourses(opts ...func(*CourseQuery)) *UserQuery {
	query := (&CourseClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOwnedCourses = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withMemberships; query != nil {
		if err := _q.loadMemberships(ctx, query, nodes,
			func(n *User) { n.Edges.Memberships = []*CourseMembership{} },
			func(n *User, e *CourseMembership) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withOwnedCourses; query != nil {
		if err := _q.loadOwnedCourses(ctx, query, nodes,
			func(n *User) { n.Edges.OwnedCourses = []*Course{} },
			func(n *User, e *Course) { n.Edges.OwnedCourses = append(n.Edges.OwnedCourses, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadMemberships(ctx context.Context, query *CourseMembershipQuery, nodes []*User, init func(*User), assign func(*User, *CourseMembership)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(coursemembership.FieldUserID)
	}
...
func (_q *UserQuery) loadOwnedCourses(ctx context.Context, query *CourseQuery, nodes []*User, init func(*User), assign func(*User, *Course)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Course(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.OwnedCoursesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
